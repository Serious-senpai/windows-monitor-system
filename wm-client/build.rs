...
impl CommonPaths {
    fn new() -> Self {
        ...
        let wm_client_dir = Path::new(&env_cargo_manifest_dir).to_path_buf();
        let workspace_dir = wm_client_dir.parent().unwrap().to_path_buf();
...
fn copy_deploy_directory(paths: &CommonPaths) {
    fs::create_dir_all(&paths.exe_dir).unwrap();

    for file in paths.deploy_dir.read_dir().unwrap() {
        let source = file.unwrap().path();
        println!("cargo:rerun-if-changed={}", source.display());
        fs::copy(&source, paths.exe_dir.join(source.file_name().unwrap())).unwrap();
    }
}
